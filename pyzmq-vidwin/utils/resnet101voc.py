...
def create_resnetvoc(layer_name, number):
    base_model=ResNet101(weights='imagenet',include_top=False) #imports the mobilenet model
    ...
    model.save('resnet101_model_voc_20class_ep_50_sgd_layer_'+str(len(model.layers))+'.h5')
...
for number in layer:
    ...
    print(layer_name)
    create_resnetvoc(layer_name, number)