...
def fixed_batcher(inp_q, out_q, query_predicates):
    ...
    while True:
        try:
            new_frame = inp_q.get(timeout=0.1)
            # print(new_frame)
            if len(frames) == batchsize:  # or new_frame[2] == 1:
                # put random batches
                #idx = random.randint(5, 15)
                # out_q.put(frames[:int(idx/2)] + frames[int(3*idx/2):])
                # diff_batch = create_diff_batch(fra
                # mes) # send only diff of batch values
                ...
                # frames.append(batchsize) # for logging on sub about batch size
                out_q.put(frames)
                # out_q.put(diff_batch)
                #print('put')
                frames = []
